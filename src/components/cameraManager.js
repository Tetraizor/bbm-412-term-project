...
export default class CameraManager extends Component {
    ...
    update() {
        ...
        movementVector =
            forward.clone().multiplyScalar((this.keys['w'] ? 1 : 0) - (this.keys['s'] ? 1 : 0)).add(
                right.clone().multiplyScalar((this.keys['d'] ? 1 : 0) - (this.keys['a'] ? 1 : 0)));
        ...
        if (this.gameObject.transform.position.x < -9) this.gameObject.transform.position.x = -9;
        if (this.gameObject.transform.position.x > 9) this.gameObject.transform.position.x = 9;
        if (this.gameObject.transform.position.z < -9) this.gameObject.transform.position.z = -9;
        if (this.gameObject.transform.position.z > 9) this.gameObject.transform.position.z = 9;
    ...
    projectVector(velocity, forward) {
        ...
        const projection = forwardVec.clone().multiplyScalar(velocityVec.dot(forwardVec));
